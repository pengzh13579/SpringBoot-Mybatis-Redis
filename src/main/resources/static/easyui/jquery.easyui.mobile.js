...
(function ($) {
  ...
  $.fn.navpanel.parseOptions = function (_5) {
    return $.extend({}, $.fn.panel.parseOptions(_5),
...
(function ($) {
      ...
      $(document).unbind(".mobile").bind("click.mobile", function (e) {
        ...
        if (a.length) {
          var _8 = $.parser.parseOptions(a[0],
              ["animation", "direction", {back: "boolean"}]);
          if (_8.back) {
            $.mobile.back();
            e.preventDefault();
          } else {
  ...
  $.map(
      ["validatebox", "textbox", "passwordbox", "filebox", "searchbox", "combo",
        "combobox", "combogrid", "combotree", "combotreegrid", "datebox",
      ...
      function (_1a) {
        if ($.fn[_1a]) {
          $.extend($.fn[_1a].defaults,
              {iconWidth: 56, spinAlign: "horizontal"});
        }
      });
  if ($.fn.menu) {
